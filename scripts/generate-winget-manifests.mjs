...
//
// Run from CLI: VERSION=0.2.3 PUBLISH_OWNER=udonthavemotion node scripts/generate-winget-manifests.mjs
...
export function generateWingetManifests({
  ...
  const installerUrl = `https://github.com/${publishOwner}/zeromotion-one/releases/download/v${version}/${name}-${version}-setup.exe`;
  const releaseNotesUrl = `https://github.com/${publishOwner}/zeromotion-one/releases/tag/v${version}`;
...
if (isCli) {
  ...
  const result = generateWingetManifests({
    ...
    name: pkg.name,
    publishOwner: process.env.PUBLISH_OWNER || "udonthavemotion",
  });
